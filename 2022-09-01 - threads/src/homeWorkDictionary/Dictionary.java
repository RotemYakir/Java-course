...
public class Dictionary {
	 ...
	 */
	public void addEntry(String entry, String definition) throws DictionaryExceptions {
		if (entry == null) {
			throw new DictionaryExceptions("add entry failed - entry cannot be null");
		}
		if (entry == "") {
			throw new DictionaryExceptions("add entry failed - entry cannot be of 0 characters");
		}
		if (map.containsKey(entry)) {
			throw new DictionaryExceptions("add entry failed - entry already exists");
		}
	 ...
	 */
	public void editEntry(String entry, String definition) throws DictionaryExceptions {
		if (!map.containsKey(entry)) {
			throw new DictionaryExceptions("editEntry failed - entry not exists in dictionary");
		}
		map.put(entry, definition);
	}
